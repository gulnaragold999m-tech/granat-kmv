...
def follow_up(channel, lead_id, name, tg_ready):
    ...
    # Запасной ход. Telegram и WhatsApp у клиента могут не открыться — из
    # России они сейчас ходят через VPN, и сервер об этом не узнает никогда:
    # 09.08.2026 сообщение в WhatsApp ушло с одной галочкой и не дошло.
    # ВКонтакте работает без обхода блокировок, поэтому кладём его вторым
    # выходом под основную кнопку. Клиент не должен упираться в стену.
    escape = {
        "url": VK_WRITE_LINK,
    ...
    # Telegram нас не принял, а другого канала клиент не выбирал. Молчать
    # нельзя: даём ВКонтакте как основной путь.
    return {
        "url": VK_WRITE_LINK,
        "label": "Написать во ВКонтакте →",
        "note": f"Напишите нам в сообщения сообщества — ответим там же, "
                f"по заявке №{lead_id}.",
    }
