...
const fajl = join(koren, 'adres-medalon.html');
...
const podoshlo = await s.evaluate(() => {
  ...
  for (let kegl = 4.4; kegl >= 2.9; kegl -= 0.1) {
...
await s.evaluate(() => document.fonts.ready);
await s.waitForTimeout(400);
await s.pdf({ path: join(koren, 'adres-medalon.pdf'), format: 'A4', printBackground: true,
              margin: { top: 0, right: 0, bottom: 0, left: 0 } });
await s.screenshot({ path: join(koren, 'adres-medalon-predprosmotr.png') });
await b.close();
