...
@app.route("/z")
def korotkiy_adres_zayavki():
    """Короткий адрес для телефонного разговора: granat-kmv.ru/z

    Заведён 21.08.2026. Владелица: «Мне позвонили, а я прошу — перейдите
    на сайт и заполните заявку. Я не знаю, как это сделать».

    Продиктовать голосом «гранат дефис кмв точка ру, там внизу форма»
    человек не запишет без ошибки. «Наш сайт, слэш, зэ» — запишет.
    Открывается сразу форма заявки, а не главная.

    Метка utm_source=zvonok отделяет пришедших со звонка от остальных:
    через неделю будет видно, сколько заявок приносит сам разговор.
    """
    return redirect("/?utm_source=zvonok&utm_medium=phone#zayavka", code=302)
