...
# Карта сайта: адрес, дата последней ПРАВКИ, частота, приоритет.
#
# ДАТЫ СТАВЯТСЯ РУКАМИ, И ЭТО СОЗНАТЕЛЬНО. Соблазн подставить сегодняшнее
# число во все строки велик, но это враньё роботу: он видит, что сайт
# «меняется» каждый день, а содержимое прежнее, и перестаёт верить дате
# вообще. Меняете страницу — меняете здесь дату, одной строкой.
#
# Значения взяты из карты, которая уже лежала на живом сайте (снята
# владелицей 21.08.2026), плюс пять новых страниц услуг. Даты 21.08
# стоят там, где правки действительно были: главная и /pechat.
KARTA_SAJTA = [
    ("/",                "2026-08-21", "weekly",  "1.0"),
    ("/pechat",          "2026-08-21", "weekly",  "0.9"),
    ("/cifra",           "2026-08-08", "weekly",  "0.9"),
    ("/raboty",          "2026-08-08", "weekly",  "0.8"),
    ("/kak-rabotaem",    "2026-08-08", "monthly", "0.6"),
    ("/kontakty",        "2026-08-08", "monthly", "0.7"),
    ("/privacy.html",    "2026-08-03", "yearly",  "0.1"),
    ("/consent.html",    "2026-08-03", "yearly",  "0.1"),
]

# Страницы услуг приезжают сюда сами: добавили запись в uslugi.py —
# она появилась в карте. Приоритет 0.8: ниже раздела, выше служебных.
KARTA_USLUG_DATA = "2026-08-21"
...
def sitemap():
    ...
    До 21.08.2026 здесь стояла отдача файла с диска. Файл существует
    и отдаётся (проверено владелицей), но живёт только в Amvera:
    в репозитории его нет, и при чистой выкатке он теряется. Плюс
    новые страницы приходится дописывать в него руками, а значит
    рано или поздно не допишут.

    ЧТО ВАЖНО НЕ ПОТЕРЯТЬ: в старом файле были /privacy.html
    и /consent.html. Первая версия этой функции их выкинула — они
    обязаны быть по 152-ФЗ и уже проиндексированы. Возвращены,
    и список ниже полный.
    """
    xml = ['<?xml version="1.0" encoding="UTF-8"?>',
           '<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">']

    def stroka(put, data, chastota, prioritet):
        return ("  <url>"
                "<loc>https://granat-kmv.ru%s</loc>"
                "<lastmod>%s</lastmod>"
                "<changefreq>%s</changefreq>"
                "<priority>%s</priority>"
                "</url>" % (put, data, chastota, prioritet))

    for put, data, chastota, prioritet in KARTA_SAJTA:
        xml.append(stroka(put, data, chastota, prioritet))
        # Страницы услуг идут сразу за своим разделом: так карта читается
        # человеком, а не только роботом.
        if put == "/pechat":
            for u in uslugi.spisok():
                xml.append(stroka("/pechat/" + u["slug"], KARTA_USLUG_DATA, "monthly", "0.8"))
