...
# ── ОТЗЫВЫ И РЕЙТИНГ ЯНДЕКСА ────────────────────────────────────────────
# Отзывы живут в карточке Яндекс Бизнеса, а не у нас. Сайт показывает
# две вещи: официальный бейдж с рейтингом (его рисует сам Яндекс, цифру
# подделать нельзя) и тексты отзывов, которые владелица разрешила
# перепечатать.
#
# Номер карточки читается из переменной окружения, а не вписан в код,
# по одной причине: карточку заводит Гульнара, и вписать номер в панели
# Amvera она может сама, без выкладки файлов. Переменные Amvera читаются
# только при старте — после ввода приложение надо перезапустить.
#
# Пока номера нет и отзывы не перепечатаны, блока на сайте нет вовсе.
# Показывать «5.0» и придуманные отзывы нельзя: это то самое враньё
# клиенту, от которого мы отказываемся в остальных местах.


def nomer_kartochki(text):
    """Номер карточки из чего угодно: из самого числа, из ссылки на Яндекс
    Карты, из целиком вставленного кода бейджа.

    Разобрать что дали дешевле, чем объяснять в переписке, какую именно
    часть ссылки копировать. Но «взять все цифры подряд» не годится: в коде
    бейджа рядом с номером стоят ширина и высота рамки, и номер выходит
    длиннее настоящего на пять знаков. Поэтому сначала ищем номер там, где
    он стоит у Яндекса всегда — сразу после rating-badge/, /org/ или
    /profile/, — и только потом гадаем.
    """
    s = str(text or "").strip()
    if s.isdigit():
        return s

    m = re.search(r"(?:rating-badge/|/org/(?:[^/]+/)?|/profile/)(\d{6,})", s)
    if m:
        return m.group(1)

    # Ссылка непривычного вида. Берём самое длинное число: номер карточки
    # у Яндекса десятизначный, и в адресе он почти всегда длиннее всего
    # остального. Ошибётся — бейдж не покажет ничего, и это заметно сразу.
    cifry = re.findall(r"\d{6,}", s)
    return max(cifry, key=len) if cifry else ""


_YANDEX_SYRYO = os.getenv("YANDEX_ORG_ID", "").strip()
YANDEX_ORG_ID = nomer_kartochki(_YANDEX_SYRYO)


def _ssylka_na_kartochku():
    """Куда ведёт кнопка «Читать отзывы».

    Порядок важен. Сначала переменная YANDEX_ORG_URL — ею можно перебить
    что угодно, не трогая код. Потом то, что вписано в YANDEX_ORG_ID, если
    это готовая ссылка: раз её скопировали из Яндекса, она точно рабочая,
    и собирать свою поверх неё незачем. И только в последнюю очередь —
    собранная из номера: короткий адрес без названия организации Яндекс
    открывает переадресацией, но проверять это надо кликом, а не на веру.
    """
    yavnaya = os.getenv("YANDEX_ORG_URL", "").strip()
    if yavnaya:
        return yavnaya
    if _YANDEX_SYRYO.startswith("http"):
        return _YANDEX_SYRYO
    if YANDEX_ORG_ID:
        return f"https://yandex.ru/maps/org/{YANDEX_ORG_ID}/reviews/"
    return ""


YANDEX_ORG_URL = _ssylka_na_kartochku()

# Перепечатанные отзывы. Заполняем ТОЛЬКО настоящими: текст слово в слово
# из карточки, имя — как подписан автор там же. Свои слова за клиента не
# сочиняем: отзыв, которого нет в Яндексе, любой посетитель разоблачает
# в один клик по кнопке под блоком.
#
# Формат записи, все поля кроме первого необязательные:
#   {"tekst": "Приглашения сделали за два дня, гости спрашивали, где печатали",
#    "imya": "Марина",            — как автор подписан в Яндексе
#    "data": "июль 2026",         — месяц отзыва
#    "povod": "свадебные приглашения"}  — по какой работе отзыв
OTZYVY = [
    # Пусто до тех пор, пока Гульнара не пришлёт настоящие тексты
    # из карточки. Пустой список — это не «забыли»: блок отзывов
    # молча не показывается, и сайт остаётся честным.
]


@app.context_processor
def otzyvy_dlya_shablonov():
    """Отдаёт отзывы и рейтинг во все шаблоны сразу.

    Через контекст, а не параметром render_template: ссылка на карточку
    стоит в подвале, а подвал общий для всех страниц. Иначе пришлось бы
    добавлять её в каждый route и однажды забыть.
    """
    return {
        "yandex_org_id": YANDEX_ORG_ID,
        "yandex_org_url": YANDEX_ORG_URL,
        "otzyvy": OTZYVY,
        # Показывать ли блок вообще. Хоть бейдж, хоть тексты — что-то
        # настоящее должно быть, иначе на странице повиснет заголовок
        # «Отзывы» над пустотой.
        "otzyvy_pokazyvat": bool(YANDEX_ORG_ID or OTZYVY),
    }
...
def health():
    ...
    return jsonify(
        ...
        # Подхватился ли номер карточки Яндекса. Переменные Amvera
        # читаются только при старте, и «вписала, а бейджа нет» —
        # это обычно забытый перезапуск. Здесь видно за секунду,
        # без похода в панель хостинга.
        yandex_card=(
            f"карточка {YANDEX_ORG_ID}, отзывов на сайте {len(OTZYVY)}"
            if YANDEX_ORG_ID else "номер карточки не задан (YANDEX_ORG_ID)"
        ),
    )
